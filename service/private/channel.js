...
class __private_channel extends Entity {
  ...
  async _classify_staged_attachment(attachment, folder_attachment) {
    ...
    for (let nid of toArray(attachment)) {
      let rows = await this.db.await_query(
        'SELECT id, parent_id, owner_id, origin_id, category FROM media WHERE id=?', `${nid}`
      );
      ...
      if (`${node.owner_id}` !== `${this.uid}` && `${node.origin_id}` !== `${this.uid}`) {
  ...
  async _promote_staged_to_folder(deviceNids, folderNid) {
    ...
    let data = await this.db.call_proc('mfs_move_all', stringify(src), this.uid, folderNid, hub_id);
    data = toArray(data);
    const remap = {};
    for (let node of data) {
  ...
  async _confirm_promoted(nids, folderNid) {
    ...
    for (let nid of toArray(nids)) {
      try {
        let rows = await this.db.await_query(
          'SELECT id FROM media WHERE id=? AND parent_id=?', `${nid}`, `${folderNid}`
        );
        if (!isEmpty(toArray(rows))) confirmed.push(`${nid}`);
      } catch (e) {
        this.warn('channel.post: promotion confirm failed', nid, e && e.message);
      }
    }
    return confirmed;
  }
  ...
  async _purge_staged_copies(nids) {
    ...
    for (let nid of nids) {
      try {
        ...
        this.warn('channel.post: failed to purge staged copy', nid, e && e.message);
      }
    }
  }
  ...
  async _notify_folder_new_nodes(nids, folderNid) {
    if (isEmpty(nids)) return;
    const hub_id = this.hub.get(Attr.id);
    let recipients = toArray(await this.yp.await_proc('entity_sockets', { hub_id }));
    if (isEmpty(recipients)) return;
    for (let nid of nids) {
      const nodes = {};
      for (let r of recipients) {
        if (!r || !r.uid) continue;
        let attr = nodes[r.uid] ||
          (await this.db.await_proc('mfs_access_node', r.uid, nid));
        nodes[r.uid] = attr;
        if (isEmpty(attr) || !attr.nid) continue;
        try {
          await RedisStore.sendData(this.payload(attr, { service: 'media.new' }), r);
        } catch (e) {
          this.warn('channel.post: media.new notify failed', nid, e && e.message);
        }
      }
    }
  }
  ...
  async post() {
    ...
    if (copy_only && !isEmpty(attachment)) {
      staged = await this._classify_staged_attachment(attachment, folder_attachment);
      if (!isEmpty(staged.device)) {
        ...
        if (!isEmpty(folder) && (Number(folder.privilege) & MFS_PERM_WRITE)) {
          const remap = await this._promote_staged_to_folder(staged.device, nid);
          ...
          const confirmed = new Set(await this._confirm_promoted(promoted, nid));
          const failed = [];
          ...
          if (!isEmpty(failed)) {
            this.warn('channel.post: promotion unconfirmed, purging from staging', failed);
            staged.workspace.push(...failed);
          }
        } else {
          this.warn('channel.post: caller lacks write on folder, staged uploads stay sbox-only', nid);
